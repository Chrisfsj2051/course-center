      ...
      <el-container class="post">
        ...
        <el-main >
          <div class="reply_post_content">
            ...
            <span v-if=item.reply_to>{{item.reply_content}}</span>
          </div>
          <div class="post_content">
        ...
        <el-aside  width="12%">
          <el-button type="primary" size="mini" @click="reply(item.reply_id)" class="post_right_button">回复</el-button>
          <div class="post_right">
            <p>{{item.reply_time}}</p>
  ...
  import {reqPostDetail,replyPost,deletePost}from '../../../api'
    export default {
      ...
      methods:{
        ...
        async handleEdit(){
            if(this.content===''){
              this.$message.error("回复内容不为空,且不能只为空白字符")
            }
            else{
        ...
        delete_post(){
          this.$confirm('确认删除该帖子吗，删除后将无法恢复, 是否继续?', '提示', {
           ...
           const result=await deletePost(this.tie_id)
            ...
            this.$router.replace('/center/forum/postlist')}
            else{
...
//初始化界面
      async mounted() {
          ...
          const result=await reqPostDetail(this.tie_id,this.currentPage)
          this.post=result
        if(this.$store.state.id==this.post.id||this.$store.state.status===-1)
          this.visible=true;
      }
    }
</script>
